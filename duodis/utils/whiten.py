...
def whitenapply_cm(X, m, P, dimensions=None):
    ...
    for num in range(len(X)):
        if num == 0:
            ...
            X_ = np.concatenate((X_,X[num]), axis = 1)
        for a in range(X[num].shape[1]):
            vec_labels.append(num)
        print('\r>>>> {}/{} done...'.format((num+1), len(X)), end='')
    print('')

    X_ = np.dot(P[:dimensions, :], X_-m)
    X_ = X_ / (np.linalg.norm(X_, ord=2, axis=0, keepdims=True) + 1e-6)
    X = []
    for num in range(len(np.unique(vec_labels))):
