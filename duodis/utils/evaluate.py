...
def rank_trans(score_ori, vec_labels):
    num_db = len(np.unique(vec_labels))
    scores = np.zeros((num_db,score_ori.shape[1]))
    for num in range(num_db):
        index = np.where(np.asarray(vec_labels) == num)
        if len(index[0]) == 1:
            scores[num,:] = score_ori[index[0],:]
        else:
            scores[num,:] = np.max(score_ori[index[0],:],0)
        if (num + 1) / 5000 == 0:
            print('\r>>>> {}/{} done...'.format((num+1), num_db), end='')
    print('')
    ranks = np.argsort(-scores, axis=0)
    return ranks

def cross_matching(vecs,qvecs):
    scores = np.zeros((len(vecs),len(qvecs)))
    for num_db in range(len(vecs)):
        for num_q in range(len(qvecs)):
            score = np.dot(vecs[num_db].T, qvecs[num_q])
            if score.shape[0] == 1:
                scores[num_db,num_q] = np.sum(score,0)
            else:
                scores[num_db,num_q] = np.sum(np.max(score,0))
            print('\r>>>> {}/{} done...'.format((num_db+1), len(vecs)), end='')
    print('')
    return scores
