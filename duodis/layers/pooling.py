...
class RMATCH(nn.Module):

    def __init__(self, L=4, eps=1e-6):
        super(RMATCH,self).__init__()
        self.L = L
        self.eps = eps

    def forward(self, x):
        return LF.rmac(x, L=self.L, eps=self.eps, sepa=True)
...
class CroW(nn.Module):
    ...
    def __init__(self, spatial_a = 2, spatial_b = 2):
        super(CroW, self).__init__()
        self.spatial_a = spatial_a
        self.spatial_b = spatial_b

    def forward(self, x):
        return LF.crow(x,spatial_a = self.spatial_a, spatial_b = self.spatial_b)

    def __repr__(self):
        return self.__class__.__name__ + '(' + 'spatial = ' + '{}_{}'.format(self.spatial_a, self.spatial_b) + ')'
